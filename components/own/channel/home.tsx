...
export default function ChannelHome({ channelData }: { channelData: ChannelDataType }) {

    const [listPo, setListPo] = useState<VideoDataType[]>([])


    useEffect(() => {
        axios.get('/api/video/viewdesc', {
            params: {
                channelId: channelData.id
            }
        }).then(res => { setListPo(res.data) })
    }, [])
        ...
        <div className="flex flex-col">
            <div className="flex flex-col gap-3">
                <div className="flex gap-5">
                    ...
                    <p className="flex gap-2 items-center"><FaPlay /> Phát cmn hết</p>
                </div>
                <div className="overflow-x-scroll w-full grid grid-cols-5 grid-flow-col-dense h-max">
                    {listPo.map((video, index) => {
                        return (
                            <VideoItem videoData={video} key={index} />
                        )
                    })}
